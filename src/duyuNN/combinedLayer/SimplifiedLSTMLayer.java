...
public class SimplifiedLSTMLayer implements NNInterface, Serializable {
    ...
    public SimplifiedLSTMLayer(LinearLayer xseedInputLinear, LinearLayer xseedForgetLinear,
        LinearLayer xseedCandidateStatelinear, int xHiddenLength) throws Exception {
        hiddenLength = xHiddenLength;

        if (!(hiddenLength == xseedInputLinear.inputLength / 2 && hiddenLength == xseedInputLinear.outputLength
            && hiddenLength == xseedForgetLinear.inputLength / 2 && hiddenLength == xseedForgetLinear.outputLength
    ...
    public void forward() {
        ...
        // link manually
        System.arraycopy(connectInputHistory.output, 0, inputLinear.input, 0, hiddenLength * 2);
        System.arraycopy(connectInputHistory.output, 0, forgetLinear.input, 0, hiddenLength * 2);
        System.arraycopy(connectInputHistory.output, 0, candidateStateLinear.input, 0, hiddenLength * 2);
    ...
    public void backward() {
        ...
        for (int i = 0; i < 2 * hiddenLength; i++) {
            connectInputHistory.outputG[i] =
                inputLinear.inputG[i] + forgetLinear.inputG[i] + candidateStateLinear.inputG[i];
        }
    ...
    public Object cloneWithTiedParams() {
        ...
        try {
            clone = new SimplifiedLSTMLayer(inputLinear, forgetLinear, candidateStateLinear, hiddenLength);
        } catch (Exception e) {
