...
public class LookupLayer implements NNInterface {
    ...
    public LookupLayer(int xEmbeddingLength, int xVocabSize, int xInputLength) {
        this(xEmbeddingLength, xVocabSize, xInputLength, 0);
    }

    public LookupLayer(int xEmbeddingLength, int xVocabSize, int xInputLength, int xLinkId) {
    ...
    public void updateAdaGrad(double learningRate, int batchsize, boolean isNorm) {
        // TODO Auto-generated method stub
        for (int wordId : changeMap.keySet()) {
            double[] embedG = changeMap.get(wordId);
            double tmpL2Norm = 0.0;
            ...
            for (int j = 0; j < embeddingLength; ++j) {
                adaLR[wordId][j] += (embedG[j] / batchsize) * (embedG[j] / batchsize);
            }

            for (int j = 0; j < embeddingLength; j++) {
                table[wordId][j] += learningRate * embedG[j] / (batchsize * Math.sqrt(adaLR[wordId][j]));
    ...
    public void setEmbeddings(double[][] embed) throws Exception {
        if (embed == null || embed.length != table.length || embed.length == 0) {
