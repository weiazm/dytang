...
public class DocSimplifiedLSTM implements NNInterface {
    ...
    public DocSimplifiedLSTM(LookupLinearTanh seedLLT1, LookupLinearTanh seedLLT2, LookupLinearTanh seedLLT3,
        int[][] wordIdMatrix, SimplifiedLSTMLayer seedSimplifiedLSTM) throws Exception {
        hiddenLength = seedLLT1.outputLength;
        sentenceConvList = new ArrayList<SentenceConvMultiFilter>();

        int maxWindowSize =
            Math.max(seedLLT1.lookup.inputLength, Math.max(seedLLT2.lookup.inputLength, seedLLT3.lookup.inputLength));

        for (int i = 0; i < wordIdMatrix.length; i++) {
            if (wordIdMatrix[i].length >= maxWindowSize) {
                sentenceConvList.add(new SentenceConvMultiFilter(wordIdMatrix[i], seedLLT1, seedLLT2, seedLLT3));
            } else {
                continue;
            }
        }
